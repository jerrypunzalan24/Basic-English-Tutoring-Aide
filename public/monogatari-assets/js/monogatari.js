...
$_ready(function() {
	...
	$_("body").on("click", "[data-do]", function() {
		...
		if ($_(this).data("do") != "null" && $_(this).data("do") != "") {
			var back = ["show", "play", "display", "hide", "scene", "stop", "pause", "finish"];
			try {
		...
		try {
			...
			switch (typeof statement) {
				case "string":
					...
					switch (parts[0]) {
						case "finish":
							window.open("../complete_lesson.php?id="+userId+"&lessonId="+lessonId+"&score="+score, "", "width=200,height=100");
							score = 0; //CGCGGCGCG
							next();
							break;
						case "change":
						...
						case "jump":
							...
							whipeText();
							if(parts[1] == "Possessive")
								lessonId = 3;
							if(parts[1] == "Pronouns")
								lessonId = 4;
							if(parts[1] == "BeVerbs")
								lessonId = 5;
							if(parts[1] == "ActionVerbs")
								lessonId = 6;
